import { boolean, foreignKey, index, pgTable, text, timestamp, uniqueIndex } from "drizzle-orm/pg-core";
...
export const commentReaction = pgTable("comment_reactions", {
    ...baseEntity,
...
export const mention = pgTable("mentions", {
    ...baseEntity,
    commentId: text("comment_id").notNull().references(() => comment.id, { onDelete: "cascade" }),
    mentionedUserId: text("mentioned_user_id").notNull().references(() => user.id, { onDelete: "cascade" }),
    mentionedByUserId: text("mentioned_by_user_id").notNull().references(() => user.id, { onDelete: "cascade" }),
